...
function makeSearchProvider(getRuntime) {
    return {
        id: 'omniroute',
        available() {
            const r = getRuntime();
        ...
        async search(req, signal) {
            ...
            const body = { query: req.query, max_results: req.maxResults ?? r.maxResults };
            if (r.provider)
...
async function omniSearchProviders(ctx, runtime) {
...
export function apply(ctx, config) {
    ...
    let currentSection = () => resolveSearch(undefined);
    ...
    installSettingsSection(ctx, SEARCH_NS, searchSectionSchema, resolveSearch(undefined), {
        setSource: (src) => {
            currentSection = () => src();
        },
        onChange: () => { },
        ...
        handler: (req, res) => {
            void (async () => {
                try {
                    ...
                    if (runtime.provider)
                        requestBody.provider = runtime.provider;
                    const headers = { 'Content-Type': 'application/json' };
                    if (key)
                        headers['Authorization'] = `Bearer ${key}`;
