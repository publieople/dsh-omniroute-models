...
- **联网搜索 provider**（`src/index.ts`）：插件自有 `omniroute-models` settings 命名空间存搜索配置，开启后向 `ctx.web` 注册 `WebSearchProvider`（id `omniroute`），DSH 的 `web_search` 即走 OmniRoute；配置/测试路由 `GET|POST /api/search-config`、`POST /api/search-test`。该区段经 `installSettingsSection` 注册（会等待 `settings` 服务就绪），插件激活顺序不会与之竞态；`available()` 只要求 `enabled` + http(s) 网关地址，key 按需惰性解析。
...
- 生产 bundle（GitHub，固定版本）：`dsh plugin --profile web add github:publieople/dsh-omniroute-models#v0.2.4`，然后重启目标 profile。
